...
import { deletepacsurlParameter, PacsurlParameterState } from '@icode-tfs-ngrx-demo/pacsurl-domain';
...
parameterComponent:boolean=false;
  constructor(private router: Router,private store: Store<{ pacsurlsParameter: PacsurlParameterState }>) {}
  ngOnInit() {
  ...
  deletePacsPopup(parameterId: any) {
    const confirmed = window.confirm('Are you sure you want to delete this pacs Parameter?');
    if (confirmed) {
